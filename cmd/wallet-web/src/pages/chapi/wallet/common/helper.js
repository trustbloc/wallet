...
const ALL_ICONS = ['account_box', 'contacts', 'person', 'person_outline', 'card_membership', 'portrait', 'bento', 'directions_car', 'house']
const VC_ICONS_MAP = {
...
export const getVCIcon = (type) => VC_ICONS_MAP[type] ? VC_ICONS_MAP[type] : ALL_ICONS[Math.floor(Math.random() * Math.floor(ALL_ICONS.length))]

export const isVPType = (type) => toLower(type) == 'verifiablepresentation'

export const getCredentialType = (types) => types.filter(type => type != "VerifiableCredential")[0]
