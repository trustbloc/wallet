...
export class CHAPIEventHandler {
    ...
    getEventData() {
        ...
        let {query, challenge, domain, invitation, credentials} = this.credEvent.credentialRequestOptions.web.VerifiablePresentation
    ...
    sendError(e) {
        this.credEvent.respondWith(new Promise(function (resolve, reject) {
            return reject(e);
        }))
    }
}
...
export class CHAPIHandler {
    ...
    async install(name) {
        ...
        const registration = await this.webCredHandler.installHandler({url: `${__webpack_public_path__}worker`})

        await registration.credentialManager.hints.set(
            'edge', {name, enabledTypes}
            );
    }
