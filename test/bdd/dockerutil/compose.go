...
func (c *Composition) issueCommand(args []string, dir string) (_ []byte, err error) {
	...
	cmd := exec.Command(dockerComposeCommand, cmdArgs...) // nolint:gosec // dockerComposeCommand is a constant
	cmd.Dir = dir
...
func (c *Composition) GenerateLogs(dir, logName string) error {
	...
	f, err := os.OpenFile(path.Clean(logName), os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0o600)
	if err != nil {
		return err
	}
