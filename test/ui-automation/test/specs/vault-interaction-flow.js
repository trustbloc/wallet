...
describe('TrustBloc Wallet - Add/Rename/Delete Vault flow', function () {
  const ctx = {
  ...
  it(`User dismisses welcome message banner`, async function () {
    ...
    await welcomeMessageCloseButton.waitForClickable();
  ...
  it(`User successfully adds vaults`, async function () {
    for (const [key, value] of vault.entries()) {
      ...
      const vaultCard = await $(`#vault-card-${value.name.replaceAll(' ', '-')}`);
  ...
  it('User successfully renames vaults', async function () {
    for (const [key, value] of vault.entries()) {
      ...
      const vaultCard = await $(`#vault-card-${value.name.replaceAll(' ', '-')}-renamed`);
      await expect(vaultCard).toExist();
    }
  });
