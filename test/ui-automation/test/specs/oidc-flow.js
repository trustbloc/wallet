...
describe("TrustBloc Wallet - OIDC save multiple credential flow", async function () {
  ...
  it(`User offered to save multiple credential through OIDC Issuance: already signed-in`, async function () {
    ...
    await Promise.all([credentialTypes.waitForExist(), manifestIDs.waitForExist(), credentialManifests.waitForExist(), credentialsToIssue.waitForExist()])

    await credentialTypes.setValue('https://w3id.org/citizenship/v1,https://www.w3.org/2018/credentials/examples/v1')
